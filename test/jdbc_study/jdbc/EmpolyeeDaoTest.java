...
public class EmpolyeeDaoTest {
	...
	public void test04DeleteEmployee() throws SQLException {
		...
		Assert.assertEquals(1, result);
		/*
		 * try { int res = dao.delelteDepartment(newDept); System.out.println(res);
		 * Assert.assertEquals(1, res); } catch (SQLException e) {
	...
	public void test03UpdateEmployee() throws SQLException {
		...
		Assert.assertEquals(1, result);
		/*
		 * Department newDept = new Department(4, "자브아아아앙", 10); try { int res =
		 * dao.updateDepartment(newDept); Assert.assertEquals(1, res); } catch
		 * (SQLException e) { System.out.println(e.getErrorCode()); }
		 */
	}
	...
	public void test05SelectEmployeeByNo() throws SQLException {
		...
		/*
		 * Department newDept = new Department(1); try { Department res =
		 * dao.selectDepartmentByNo(newDept); JOptionPane.showMessageDialog(null, res);
		 * } catch (SQLException e) { JOptionPane.showMessageDialog(null, "검색 실패"); }
		 */
	}
